...
class SHSpider:
    """
    Spider for crawling i-sh.co.kr (SH서울주택도시공사)
    SH공사 임대주택 모집공고
    """

    BASE_URL = 'https://www.i-sh.co.kr'
    LIST_URL = 'https://www.i-sh.co.kr/main/lay2/program/S1T294C297/content.do'
    DETAIL_URL = 'https://www.i-sh.co.kr/main/lay2/program/S1T294C297/view.do'
    ...
    def fetch_list_page(self, page=1, page_size=10):
        """
        Fetch announcement list from SH website
        """
        data = {
            'pageIndex': page,
            'pageUnit': page_size,
        ...
        try:
            ...
            logger.error(f'Failed to fetch SH list page {page}: {e}')
            return None

    def _parse_list_html(self, html):
        """
        Parse HTML list page from SH website
        """
        soup = BeautifulSoup(html, 'html.parser')
        items = []

        rows = soup.select('table tbody tr, .board-list li, .bbs_list tbody tr')
        for row in rows:
            try:
                cells = row.select('td')
                if len(cells) < 3:
                    # Try anchor-based parsing
                ...
                item = {
                    ...
                    'apply_start': cells[3].get_text(strip=True) if len(cells) > 3 else '',
                    'apply_end': cells[4].get_text(strip=True) if len(cells) > 4 else '',
                }

                link = row.select_one('a')
                if link:
                    href = link.get('href', '')
                    item['link'] = href if href.startswith('http') else f'{self.BASE_URL}{href}'

                items.append(item)
            except Exception as e:
                logger.warning(f'Failed to parse SH row: {e}')

        return {'items': items, 'total_count': len(items)}

    def fetch_detail_page(self, url_or_id):
        """
        ...
        try:
            ...
            if url_or_id.startswith('http'):
                ...
                url = f'{self.DETAIL_URL}?seq={url_or_id}'
            ...
            logger.error(f'Failed to fetch SH detail {url_or_id}: {e}')
            return None
    ...
    def _parse_detail_html(self, html):
        ...
        title = soup.select_one('h3, .title, .view-title, .board-view-title')
        detail['name'] = title.get_text(strip=True) if title else ''

        info_rows = soup.select('.info-table tr, .view-info tr, dl dt')
        for row in info_rows:
            ...
            if label and value:
                ...
                if '주소' in label_text or '소재지' in label_text or '위치' in label_text:
                    detail['address'] = value_text
                elif '공급유형' in label_text or '임대유형' in label_text or '주택유형' in label_text:
                    detail['housing_type_raw'] = value_text
                elif '공급세대' in label_text or '모집세대' in label_text or '세대수' in label_text:
    ...
    def _parse_supply_table(self, soup):
        ...
        """
        units = []
        tables = soup.select('table.supply-table, table.tbl_type01, .unit-table table')
        ...
        for table in tables:
            ...
            for row in rows:
                ...
                unit = {
                    ...
                    'area': self.normalize_numeric_value(cells[1].get_text(strip=True)),
                    'units': self.normalize_numeric_value(cells[2].get_text(strip=True)),
    ...
    def detect_housing_type(self, name):
        """
        Detect housing type from announcement name
        """
        for keyword, housing_type in self.HOUSING_TYPE_MAP.items():
            if keyword in name:
                return housing_type
        return 'national'
    ...
    def save_complex(self, data):
        ...
        code = data.get('code', '')
        if not code:
            code = f"sh_{data.get('name', 'unknown')[:30]}_{data.get('address', '')[:20]}"

        name = data.get('name', '')
        housing_type = data.get('housing_type_raw', '')
        housing_type = self.HOUSING_TYPE_MAP.get(housing_type, self.detect_housing_type(name))
    ...
    def crawl(self, pages=3, save=True):
        ...
        try:
            ...
            for page in range(1, pages + 1):
                logger.info(f'Fetching SH page {page}/{pages}')
                time.sleep(self.delay)
                ...
                items = data.get('items', data.get('resultList', []))
                if not items:
                ...
                for item in items:
                    try:
                        ...
                        complex_data = {
                            'code': f"sh_{item.get('announcement_no', item.get('seq', ''))}",
                            'name': item.get('name', item.get('ancNm', '')),
                            'address': item.get('address', item.get('adres', '')),
                            'housing_type_raw': item.get('housing_type', item.get('rentSeNm', '')),
                            'total_units': self.normalize_numeric_value(
                                item.get('total_units', item.get('totSplyHoCnt', 0))
                            ),
                        }
                        ...
                        recruitment_data = {
                            ...
                            'status_raw': item.get('status', item.get('pblancSttusNm', '')),
                            'apply_start': item.get('apply_start', item.get('rcritPdBgnde', '')),
                            'apply_end': item.get('apply_end', item.get('rcritPdEndde', '')),
                        ...
                        result['failed_items'] += 1
                        logger.error(f'Error processing SH item: {e}')
